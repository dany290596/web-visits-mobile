...
@Component({
  selector: 'app-editar-plantilla-notificacion',
  imports: [
    CommonModule,
    ReactiveFormsModule,
    InputTextModule,
  ...
  templateUrl: './editar-plantilla-notificacion.html',
  styleUrl: './editar-plantilla-notificacion.css',
  providers: [MessageService]
})
export class EditarPlantillaNotificacion {
    ...
    this.srvStorage.userData$
      ...
      .subscribe((data: IUsuarioAutenticado) => {
        ...
        if (this.id !== undefined && this.id !== null && this.id !== "") {
          this.srvPlantillaNotificacion.getById(this.id).subscribe((data: any) => {
            if (data.respuesta === true) {
              let plantilla: any = data.data;
              // console.log("USUARIO ::: ", JSON.stringify(plantilla));
              this.form.patchValue({
                nombre: plantilla.nombre,
                cuerpoPlantilla: plantilla.cuerpoPlantilla,
                notificarEmail: plantilla.notificarEmail === 1,
                notificarTeams: plantilla.notificarTeams === 1,
                tipoPlantillaNotificacionId: plantilla.tipoPlantillaNotificacionId,
              });
            }
          });
        }
  ...
  guardar() {
    ...
    this.srvPlantillaNotificacion.update(request, this.id).subscribe((resp: any) => {
