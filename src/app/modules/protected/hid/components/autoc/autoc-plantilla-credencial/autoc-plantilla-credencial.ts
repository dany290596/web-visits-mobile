...
@Component({
  selector: 'app-autoc-plantilla-credencial',
  imports: [
    CommonModule,
    FormsModule,
    ReactiveFormsModule,
  ...
  template: `
    <label class="form-label">Plantilla de credencial <span class="text-red-500" *ngIf="isRequired">*</span></label>
    <p-autoComplete
      ...
      dataKey="id"
      [dropdown]="true"
      placeholder="Busque plantilla de credencial"
  ...
  `,
  styleUrl: './autoc-plantilla-credencial.css',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => AutocPlantillaCredencial),
    multi: true
  }]
})
export class AutocPlantillaCredencial implements OnInit, ControlValueAccessor {
  items: any[] = [];
  filteredItems: any[] = [];
  ...
  cargarPerfiles() {
    ...
    this.srvPlantillaCredencial.getAll({ Estado: 1, PageNumber: 1, PageSize: 1000 }).subscribe((res: any) => {
      if (res?.respuesta) {
