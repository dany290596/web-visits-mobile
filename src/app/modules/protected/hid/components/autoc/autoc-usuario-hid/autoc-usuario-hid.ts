...
@Component({
  selector: 'app-autoc-usuario-hid',
  imports: [
    CommonModule,
    FormsModule,
    ReactiveFormsModule,
  ...
  `,
  styleUrl: './autoc-usuario-hid.css',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => AutocUsuarioHid),
    multi: true
  }]
})
export class AutocUsuarioHid implements OnInit, ControlValueAccessor {
  items: any[] = [];
  filteredItems: any[] = [];
  ...
  cargarPerfiles() {
    ...
    this.srvUsuarioHID.getAll({ Estado: 1, PageNumber: 1, PageSize: 1000 }).subscribe((res: any) => {
      if (res?.respuesta) {
