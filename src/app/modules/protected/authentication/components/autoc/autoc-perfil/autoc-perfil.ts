...
@Component({
  ...
  template: `
    <label class="form-label">Perfil <span class="text-red-500" *ngIf="isRequired">*</span></label>
    <p-autoComplete
  ...
  `,
  styleUrl: './autoc-perfil.css',
  providers: [{
    provide: NG_VALUE_ACCESSOR,
    useExisting: forwardRef(() => AutocPerfil),
    multi: true
  }]
})
export class AutocPerfil implements OnInit, ControlValueAccessor {
  items: any[] = [];
  filteredItems: any[] = [];
  ...
  cargarPerfiles() {
    ...
    this.perfilService.getAll({ Estado: 1, PageNumber: 1, PageSize: 1000 }).subscribe((res: any) => {
      if (res?.respuesta) {
